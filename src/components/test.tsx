...
export default function GroupedBar(): JSX.Element {
  const classes = testStyle();
  const groupedBarContainer = useRef(null);
  const [graphData, setGraph] = useState<ITest>({ nodes: [], links: [] });
  ...
  useEffect(() => {
    if (graphData && groupedBarContainer.current) {
      ...
      const svg = d3
        .select(groupedBarContainer.current)
        .attr("viewBox", `[0, 0, ${width}, ${height}]`)
        .style("background-color", "#ebebeb");

      const g = svg.append("g");

      const zoom_handler = d3.zoom().on("zoom", (event: any) => {
      ...
      simulation.on("tick", () => {
        ...
        node.attr("transform", (d) => `translate(${d.x - 8}, ${d.y})`);
      });
  ...
  return (
    <div>
      <svg ref={groupedBarContainer} width={width} height={height + 30} />
    </div>
  );
}
